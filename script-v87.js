...
function renderStreamersPage(data){
  if (document.body?.dataset?.cloudStreamers === '1') return;
...
// V87 — menu único, global e reinicializável
(() => {
  function initMenu(){
    const nav=document.getElementById('tlMainNav');
    const button=document.querySelector('.tl-mobile-menu-button');
    const backdrop=document.querySelector('.tl-mobile-menu-backdrop');
    if(!nav||!button||!backdrop||button.dataset.ready==='1') return;
    ...
    function openMenu(){
      ...
      document.body.classList.add('tl-mobile-menu-open');
    }

    button.addEventListener('click',event=>{
      event.preventDefault();
      event.stopPropagation();
      nav.classList.contains('is-mobile-open')?closeMenu():openMenu();
    });

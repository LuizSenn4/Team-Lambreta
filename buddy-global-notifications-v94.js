...
const SOUND_SRC='assets/sounds/buddy-message.wav';
const soundKey=id=>`tl_buddy_sound_${uid||'anon'}_${id}`;
const soundEnabled=id=>localStorage.getItem(soundKey(id))==='1';
let audio=null;
function playMessageSound(senderId){if(!soundEnabled(senderId))return;try{audio=audio||new Audio(SOUND_SRC);audio.currentTime=0;audio.volume=.55;audio.play().catch(()=>{})}catch(_){}}
...
async function init(){ensureUi();const {data}=await sb.auth.getSession();uid=data.session?.user?.id;if(!uid)return;await refresh();channel=sb.channel(`buddy-global-${uid}-${Date.now()}`).on('postgres_changes',{event:'INSERT',schema:'public',table:'private_messages',filter:`receiver_id=eq.${uid}`},payload=>{const r=payload.new||{};if(r.read_at)return;unread+=1;paint();playMessageSound(r.sender_id);toast(r)}).on('postgres_changes',{event:'UPDATE',schema:'public',table:'private_messages',filter:`receiver_id=eq.${uid}`},refresh).subscribe();window.addEventListener('focus',refresh);window.addEventListener('online',refresh);document.addEventListener('visibilitychange',()=>!document.hidden&&refresh());window.addEventListener('buddy:messages-read',refresh)}
window.TeamBuddyNotifications={soundEnabled,setSound:(id,on)=>{localStorage.setItem(soundKey(id),on?'1':'0');window.dispatchEvent(new CustomEvent('buddy:sound-changed',{detail:{id,on}}));if(on){try{audio=audio||new Audio(SOUND_SRC);audio.volume=0;audio.play().then(()=>{audio.pause();audio.currentTime=0;audio.volume=.55}).catch(()=>{})}catch(_){}}}};
init();
})();

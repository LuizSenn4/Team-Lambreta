(() => {
  ...
  const canEditGame = () => ['master', 'admin', 'moderator'].includes(String(document.body.dataset.userRole || '').toLowerCase());
  ...
  const loadStreamer = async () => {
    ...
    const { data, error } = await liveSb
      .from('streamers')
      .select('id,display_name,main_game,tiktok_url,live_url,is_published,is_archived')
  ...
  gameForm?.addEventListener('submit', async event => {
    ...
    if (!nextGame) {
      gameFeedback.textContent = 'Escreve o nome do jogo.';
      return;
    }
    const saveButton = document.getElementById('liveGameSave');
    if (saveButton) saveButton.disabled = true;
    gameFeedback.textContent = 'A guardar…';
    const { data, error } = await liveSb.rpc('tl_set_streamer_live_game', {
    ...
    setGame(typeof data === 'string' && data ? data : nextGame);
    gameFeedback.textContent = 'Jogo atualizado.';

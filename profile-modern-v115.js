(() => {
  ...
  const covers = {
    ...
    cover_competitive:'assets/profile-covers/cover-competitive.png', cover_cyber_gamer:'assets/profile-covers/cover-cyber-gamer.png',
  ...
  const roles = profile => window.TeamPermissions?.getVisualRoles?.(profile) || [{ key:profile?.role || 'member', label:String(profile?.role || 'MEMBRO').toUpperCase(), color:'#4d6377', icon:'◆' }];
  const verified = profile => roles(profile).some(r => ['developer','dev','master','admin','administrator','staff','moderator'].includes(String(r.key || '').toLowerCase()));
  const roleMarkup = profile => roles(profile).slice(0,3).map(role => `<span class="tl-profile-role-v106" style="--role-color:${esc(role.color || '#4d6377')}"><span class="role-icon" aria-hidden="true">${esc(role.icon || '◆')}</span><b>${esc(role.label || role.key)}</b></span>`).join('');

  async function relationFor(viewerId,userId) {
    if (viewerId === userId) return { state:'self' };
    const pair = `and(requester_id.eq.${viewerId},addressee_id.eq.${userId}),and(requester_id.eq.${userId},addressee_id.eq.${viewerId})`;
    const [relationResult,blockResult,requestResult] = await Promise.all([
      sb.from('buddy_relations').select('*').or(pair).maybeSingle(),
      sb.from('user_blocks').select('*').or(`and(blocker_id.eq.${viewerId},blocked_id.eq.${userId}),and(blocker_id.eq.${userId},blocked_id.eq.${viewerId})`).limit(1),
      sb.from('private_message_requests').select('*').or(`and(sender_id.eq.${viewerId},receiver_id.eq.${userId}),and(sender_id.eq.${userId},receiver_id.eq.${viewerId})`).maybeSingle()
    ]);
    if (blockResult.data?.length) return { state:'blocked' };
    const relation = relationResult.data || null;
    if (relation?.status === 'accepted') return { state:'buddy', relation, request:requestResult.data || null };
    if (relation?.status === 'pending' && relation.requester_id === viewerId) return { state:'sent', relation, request:requestResult.data || null };
    if (relation?.status === 'pending') return { state:'received', relation, request:requestResult.data || null };
    return { state:'none', request:requestResult.data || null };
  }
  ...
  function friendMarkup(friend) {
    ...
    return `<a class="tl-profile-friend-v106" href="profile.html?user=${encodeURIComponent(friend.id)}"><span class="tl-profile-friend-avatar-v106"><span>${avatarMarkup(friend)}</span><i class="tl-profile-friend-status-v106 is-${esc(state)}"></i></span><span class="tl-profile-friend-name-v106">${esc(friend.display_name || friend.full_name || 'Membro')}</span></a>`;
  }
  ...
  function gamesMarkup(profile,catalog,own) {
    ...
    const cards = chosen.map(slug => {
      ...
      const mark = String(item.name || item.slug || '?').trim().split(/\s+/).slice(0,2).map(w => w[0]).join('').toUpperCase();
      return `<article class="tl-profile-game-card-v106"><span class="game-mark">${esc(mark)}</span><span class="game-name">${esc(item.name)}</span></article>`;
    });
  ...
  function socialItems(profile) {
    const labels = { discord:'Discord', tiktok:'TikTok', youtube:'YouTube', twitch:'Twitch' };
    const map = new Map(Object.keys(labels).map(type => [type,{type,label:labels[type],url:'',value:''}]));
    if (profile?.discord) map.get('discord').value = String(profile.discord);
  ...
  function actionMarkup(rel,userId) {
    if (rel.state === 'self' || rel.state === 'blocked') return '';
    if (rel.state === 'buddy') return `<div class="tl-profile-hero-actions-v106"><span class="tl-profile-action-v106 is-primary">${icon.friends}<span>Amigos</span></span></div>`;
    if (rel.state === 'sent') return `<div class="tl-profile-hero-actions-v106"><span class="tl-profile-action-v106">${icon.friends}<span>Pedido enviado</span></span></div>`;
    if (rel.state === 'received') return `<div class="tl-profile-hero-actions-v106"><button class="tl-profile-action-v106 is-primary" type="button" data-accept-buddy>${icon.friends}<span>Aceitar amizade</span></button></div>`;
    const requestState = rel.request?.status || '';
    return `<div class="tl-profile-hero-actions-v106"><button class="tl-profile-action-v106 is-primary" type="button" data-add-buddy>${icon.friends}<span>Adicionar amigo</span></button><a class="tl-profile-action-v106" href="buddy.html?user=${encodeURIComponent(userId)}&request=1">${icon.message}<span>${requestState === 'pending' ? 'Solicitação enviada' : 'Enviar mensagem'}</span></a></div>`;
  }
  ...
  function render(profile,stats,catalog,friends,userId,own,rel) {
    ...
    root.innerHTML = `<section class="tl-profile-hero-v106" style="--profile-cover:url('${esc(cover)}')"><div class="tl-profile-hero-content-v106"><div class="tl-profile-avatar-wrap-v106"><div class="tl-profile-avatar-inner-v106">${avatarMarkup(profile)}</div><i class="tl-profile-presence-dot-v106 is-${esc(state)}"></i></div><div class="tl-profile-identity-v106"><div class="tl-profile-title-row-v106"><h1 class="tl-profile-title-v106">${esc(profile.display_name || 'Membro Lambreta')}</h1>${verified(profile) ? `<span class="tl-profile-verified-v106" title="Conta oficial Team Lambreta">${icon.check}</span>` : ''}</div><div class="tl-profile-presence-label-v106 is-${esc(state)}"><i></i>${esc(presenceLabel(state))}</div><div class="tl-profile-role-list-v106">${roleMarkup(profile)}</div><p class="tl-profile-quote-v106">${esc(bio)}</p></div><img class="tl-profile-crest-v106" src="img/brasao-96.webp" alt="Brasão Team Lambreta"></div>${actionMarkup(rel,userId)}</section><section class="tl-profile-content-v106"><div class="tl-profile-tabs-v106" role="tablist"><button class="tl-profile-tab-v106 is-active" type="button" data-target="#profileGames">${icon.game}<span class="tl-profile-tab-copy-v106"><strong>JOGOS</strong><small>Mais jogados</small></span></button><button class="tl-profile-tab-v106" type="button" data-target="#profileFriends">${icon.friends}<span class="tl-profile-tab-copy-v106"><strong>AMIGOS</strong><small>Lista de amigos</small></span></button><button class="tl-profile-tab-v106" type="button" data-target="#profileSocial">${icon.network}<span class="tl-profile-tab-copy-v106"><strong>REDES</strong></span></button><button class="tl-profile-tab-v106" type="button" data-target="#profileInfo">${icon.profile}<span class="tl-profile-tab-copy-v106"><strong>PERFIL</strong><small>Informações</small></span></button></div><div class="tl-profile-sections-v106"><section id="profileGames" class="tl-profile-section-v106"><div class="tl-profile-section-head-v106"><h2 class="tl-profile-section-title-v106">${icon.game}JOGOS MAIS JOGADOS</h2></div><div class="tl-profile-games-v106">${gamesMarkup(profile,catalog,own)}</div></section><section id="profileFriends" class="tl-profile-section-v106"><div class="tl-profile-section-head-v106"><h2 class="tl-profile-section-title-v106">${icon.friends}LISTA DE AMIGOS</h2><span class="tl-profile-section-link-v106">${friends.total} ${friends.total === 1 ? 'amigo' : 'amigos'}</span></div><div class="tl-profile-friends-v106">${friendCards}${more}</div></section><section id="profileSocial" class="tl-profile-section-v106"><div class="tl-profile-section-head-v106"><h2 class="tl-profile-section-title-v106">${icon.network}REDES SOCIAIS</h2></div><div class="tl-profile-social-grid-v106">${socialItems(profile).map(socialCard).join('')}</div></section><section id="profileInfo" class="tl-profile-section-v106"><div class="tl-profile-section-head-v106"><h2 class="tl-profile-section-title-v106">${icon.profile}INFORMAÇÕES</h2></div>${infoMarkup(profile,stats,own)}</section></div></section>`;

    bind(rel,userId);
  }

  function bind(rel,userId) {
    ...
    document.querySelectorAll('[data-copy-value]').forEach(button => button.addEventListener('click',async event => {
      ...
      try { await navigator.clipboard.writeText(button.dataset.copyValue); } catch (_) {}
    }));
    ...
    document.querySelector('[data-add-buddy]')?.addEventListener('click',async event => {
      const button = event.currentTarget;
      button.disabled = true;
      try {
        const session = await window.TeamAuth.getSession();
        const result = await sb.from('buddy_relations').insert({requester_id:session.user.id,addressee_id:userId,status:'pending',requester_notify:false,addressee_notify:true}).select().single();
        if (result.error) throw result.error;
        button.innerHTML = `${icon.friends}<span>Pedido enviado</span>`;
      } catch (error) { console.error('[Profile add buddy]',error); button.disabled = false; }
    });

    document.querySelector('[data-accept-buddy]')?.addEventListener('click',async event => {
      const button = event.currentTarget;
      button.disabled = true;
      try {
        const result = await sb.from('buddy_relations').update({status:'accepted',addressee_notify:false,requester_notify:true,updated_at:new Date().toISOString()}).eq('id',rel.relation.id).select().single();
        if (result.error) throw result.error;
        button.innerHTML = `${icon.friends}<span>Amigos</span>`;
      } catch (error) { console.error('[Profile accept buddy]',error); button.disabled = false; }
    });
  }

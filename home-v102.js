(() => {
  ...
  function openWatch(row,trigger){
    ...
    document.getElementById('homeWatchPlatforms').innerHTML=available.length?available.map(item=>{const destination=item.name==='TikTok'?'/auth/tiktok/start':safeUrl(item.url);return `<a class="platform-${platformClass(item.name)}" href="${esc(destination)}" target="_blank" rel="noopener noreferrer" aria-label="Assistir ${esc(row.display_name||'streamer')} no ${esc(item.name)}">${platformIcon(item.name)}<span>${esc(item.name)}</span></a>`}).join(''):'<span class="home-watch-empty">Plataformas externas em atualização.</span>';

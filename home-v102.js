(() => {
  ...
  const liveCard=row=>{
    ...
    const nextLabel=homeStreamers.length>2?'PRÓXIMAS LIVES':'PRÓXIMA LIVE';
    return `<article class="home-live-card" data-watch-streamer="${esc(row.id)}" role="button" tabindex="0" aria-label="Ver opções para ${esc(row.display_name||'streamer')}">
      ...
      <span class="home-live-copy">
        <span class="home-live-toolbar">${statusMarkup(row)}<button class="home-live-next" type="button" data-live-next aria-label="${esc(nextLabel)}" title="${esc(nextLabel)}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m7 5 8 7-8 7z"/><path d="M17 5v14"/></svg><span>${esc(nextLabel)}</span></button></span>

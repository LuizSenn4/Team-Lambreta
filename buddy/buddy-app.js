(() => {
  ...
  function contactMarkup(profile) { const p = presence(profile), unread = state.unread[profile.id] || 0; return `<article class="buddy-contact-row"><button class="buddy-contact ${state.current?.id === profile.id ? 'active' : ''}" type="button" data-open-chat="${profile.id}" aria-label="Abrir conversa com ${esc(profileName(profile))}"><span class="buddy-avatar-wrap">${avatar(profile)}<i class="buddy-dot ${p}"></i></span><span class="buddy-contact-copy"><strong>${esc(profileName(profile))}</strong><span><i class="buddy-dot ${p}"></i>${esc(statusLabel(p))}</span></span>${unread ? `<b class="buddy-unread">${unread}</b>` : ''}</button><div class="buddy-contact-actions"><button type="button" data-open-chat="${profile.id}" aria-label="Conversar com ${esc(profileName(profile))}">▤</button>${profileAction(profile, '●', 'buddy-contact-profile')}</div></article>`; }
  function renderContacts() { const all=buddies().sort((a,b)=>profileName(a).localeCompare(profileName(b),'pt')); const online=all.filter(profile=>presence(profile)==='online'); const set=(id,value)=>{const node=$(id);if(node)node.textContent=value;};set('#buddyAllCount',all.length);set('#buddyOnlineCount',online.length);const shown=state.contactFilter==='online'?online:all;$('#buddyList').innerHTML = shown.length ? shown.map(contactMarkup).join('') : `<div class="buddy-empty-list"><b>${all.length?'Nenhum contacto online.':'Ainda não adicionaste nenhum Buddy.'}</b><span>${all.length?'Volta ao filtro Todos.':'Pesquisa por nickname para encontrares a comunidade.'}</span></div>`; }

(() => {
  ...
  const state = { session: null, me: null, profiles: new Map(), relations: [], blocks: [], unread: {}, current: null, messages: [], oldest: null, hasOlder: true, tab: 'contacts', contactFilter: 'all', contactView: 'list', connection: 'SUBSCRIBED', sound: localStorage.getItem('tl_buddy_sound_v100') === 'on', searchResults: [], loading: false, loadError: '', catalog:{ games:new Map(), platforms:new Map() } };
  ...
  function identitySection(label, items, type) {
    if (!items.length) return '';
    const library=window.TeamBuddyProfileIcons;
    return `<section class="buddy-info-collection"><small>${esc(label)}</small><div class="buddy-identity-grid">${items.map(item=>library?.[type]?.(item.slug,item.label)||`<span class="buddy-identity-item"><b>${esc(item.label)}</b></span>`).join('')}</div></section>`;
  }
  function toggleInfoPanel(force) {
    const panel=$('#buddyInfo'),button=$('[data-info]');
    const desktop=matchMedia('(min-width:1101px)').matches;
    const open=force ?? (desktop ? root.classList.contains('info-collapsed') : !panel.classList.contains('open'));
    if(desktop)root.classList.toggle('info-collapsed',!open);else panel.classList.toggle('open',open);
  ...
  function updateUnread() { const total = Object.values(state.unread).reduce((a,b) => a+b,0); const badge = $('#tlHeaderUnread'); if (badge) { badge.textContent = total; badge.hidden = !total; } const requestCount = state.relations.filter(r => r.status === 'pending' && r.addressee_id === state.session.user.id).length; const rb = $('#buddyRequestCount'); if (rb) { rb.textContent = requestCount; rb.hidden = false; } }

  function contactMarkup(profile) { const p = presence(profile), unread = state.unread[profile.id] || 0, bio=profile.public_bio || profile.bio || profile.main_game || 'Sempre por perto'; return `<article class="buddy-contact-row"><button class="buddy-contact ${state.current?.id === profile.id ? 'active' : ''}" type="button" data-open-chat="${profile.id}" aria-label="Abrir conversa com ${esc(profileName(profile))}"><span class="buddy-avatar-wrap">${avatar(profile)}<i class="buddy-dot ${p}"></i></span><span class="buddy-contact-copy"><strong>${esc(profileName(profile))}</strong><span><i class="buddy-dot ${p}"></i>${esc(statusLabel(p))}</span><em>“${esc(bio)}”</em></span>${unread ? `<b class="buddy-unread">${unread}</b>` : ''}</button><div class="buddy-contact-actions"><button type="button" data-open-chat="${profile.id}" aria-label="Conversar com ${esc(profileName(profile))}">▤</button>${profileAction(profile, '●', 'buddy-contact-profile')}<button type="button" data-select-user="${profile.id}" aria-label="Mais opções">⋮</button></div></article>`; }
  function renderContacts() { const all=buddies().sort((a,b)=>profileName(a).localeCompare(profileName(b),'pt')); const counts={online:0,away:0,offline:0}; all.forEach(profile=>{const value=presence(profile);if(value==='online')counts.online++;else if(value==='away'||value==='busy')counts.away++;else counts.offline++;}); const set=(id,value)=>{const node=$(id);if(node)node.textContent=value;};set('#buddyAllCount',all.length);set('#buddyOnlineCount',counts.online);set('#buddyAwayCount',counts.away);set('#buddyOfflineCount',counts.offline);let shown=all;if(state.contactFilter==='online')shown=all.filter(profile=>presence(profile)==='online');if(state.contactFilter==='away')shown=all.filter(profile=>['away','busy'].includes(presence(profile)));if(state.contactFilter==='offline')shown=all.filter(profile=>presence(profile)==='offline');$('#buddyList').classList.toggle('grid-view',state.contactView==='grid');$('#buddyList').innerHTML = shown.length ? shown.map(contactMarkup).join('') : `<div class="buddy-empty-list"><b>${all.length?'Nenhum contacto neste estado.':'Ainda não adicionaste nenhum Buddy.'}</b><span>${all.length?'Escolhe outro filtro.':'Pesquisa por nickname para encontrares a comunidade.'}</span></div>`; }
  ...
  function renderSidebar() { const tools=$('#buddyContactTools'),note=document.querySelector('.buddy-community-note');if(tools)tools.hidden=state.tab!=='contacts';if(note)note.hidden=state.tab!=='contacts';if ($('#buddySearch').value.trim().length >= 2) return renderSearch(); state.tab === 'requests' ? renderRequests() : renderContacts(); updateUnread(); }
  ...
  function selectUser(profile, loading = false) {
    ...
    root.classList.add('chat-open'); renderSidebar(); renderChat(); syncComposerForSelection(); renderInfo(); if(friendState(profile.id)==='buddy'){restoreComposerFocus(profile.id);requestAnimationFrame(()=>root.scrollIntoView({block:matchMedia('(max-width:760px)').matches?'start':'center',behavior:'smooth'}));} enrichSelectedProfile(profile.id, version);
    return version;
  }
  ...
  function renderInfo(){
    ...
    const games=listValues(p.games).map(slug=>({slug,label:state.catalog.games.get(slug)||readableSlug(slug)}));
    if(!games.length&&p.main_game)games.push({slug:String(p.main_game).toLowerCase().replace(/\s+/g,'-'),label:p.main_game});
    const platforms=listValues(p.platforms).map(slug=>({slug,label:state.catalog.platforms.get(slug)||readableSlug(slug)}));
    const modes=listValues(p.game_modes).map(mode=>String(mode).split('::')[1]||readableSlug(mode));
    const country=window.TeamCountryCatalog?.resolve?.(p.country);
    $('#buddyInfo').innerHTML=`<div class="buddy-info-inner"><button class="buddy-info-close" type="button" data-info aria-label="Fechar informações">×</button><section class="buddy-info-identity">${avatar(p)}<h2>${esc(profileName(p))}</h2><div class="buddy-info-status"><i class="buddy-dot ${presence(p)}"></i>${statusLabel(presence(p))}</div>${rolesMarkup(p)}</section><div class="buddy-info-content">${bio?`<section class="buddy-info-about"><small>SOBRE</small><p>${esc(bio)}</p></section>`:''}${p.country?`<section class="buddy-info-country-block"><small>PAÍS</small><p>${country?`${country.flag} ${esc(country.name)}`:esc(p.country)}</p></section>`:''}${identitySection('JOGOS',games,'game')}${identitySection('PLATAFORMAS',platforms,'platform')}${modes.length?`<section class="buddy-info-modes"><small>MODOS</small><div>${modes.map(mode=>`<span>${esc(mode)}</span>`).join('')}</div></section>`:''}</div><a class="buddy-info-profile-link" href="profile.html?user=${encodeURIComponent(p.id)}">${infoIcons.profile}<span>VER PERFIL COMPLETO</span><b aria-hidden="true">→</b></a></div>`;
  }
  ...
  async function handleClick(event){const el=event.target.closest('button,a');if(!el)return;if(el.matches('[data-open-chat]')){event.preventDefault();return openChat(el.dataset.openChat);}if(el.matches('[data-select-user]')){event.preventDefault();const profile=state.profiles.get(el.dataset.selectUser)||state.searchResults.find(p=>p.id===el.dataset.selectUser);if(profile)selectUser(profile,false);return;}if(el.matches('[data-reload-chat]'))return state.current&&openChat(state.current.id);if(el.matches('[data-back]')){root.classList.remove('chat-open');return;}if(el.matches('[data-info]')){toggleInfoPanel();return;}if(el.matches('[data-focus-search]')){$('#buddySearch').focus();return;}if(el.matches('[data-focus-composer]')){$('#buddyMessageInput').focus();return;}if(el.matches('[data-load-older]'))return loadOlder();if(el.matches('[data-add-buddy]')){el.disabled=true;try{await friends.request(el.dataset.addBuddy);toast('Pedido de Buddy enviado.');await refreshData();}catch(error){toast(friendlyError(error,'Falha ao enviar o pedido.'),'error');}finally{el.disabled=false;}return;}if(el.matches('[data-respond]')){try{await friends.respond(el.dataset.relation,el.dataset.respond==='accept');toast(el.dataset.respond==='accept'?'Pedido aceite. Agora são Buddies.':'Pedido recusado.');await refreshData();}catch(error){toast(friendlyError(error,'Falha ao responder ao pedido.'),'error');}return;}if(el.matches('[data-block]')){if(confirm(`Bloquear ${profileName(state.current)}? Deixarão de poder trocar mensagens ou pedidos.`)){try{await friends.block(el.dataset.block);toast('Utilizador bloqueado.');state.current=null;await refreshData();renderChat();renderInfo();}catch(error){toast(friendlyError(error,'Falha ao bloquear.'),'error');}}return;}if(el.matches('[data-unblock]')){try{await friends.unblock(el.dataset.unblock);toast('Utilizador desbloqueado.');await refreshData();}catch(error){toast(friendlyError(error,'Falha ao desbloquear.'),'error');}return;}if(el.matches('[data-report]'))return openReport(el.dataset.report);if(el.matches('[data-dialog-close]'))return $('#buddyModal').hidden=true;if(el.matches('[data-emoji]')){const input=$('#buddyMessageInput');input.value+=el.dataset.emoji;input.focus();resizeComposer();return;}if(el.matches('[data-retry]')){const failed=state.messages.find(m=>String(m.id)===el.dataset.retry);if(failed){state.messages=state.messages.filter(m=>m!==failed);$('#buddyMessageInput').value=failed.body;sendMessage();}return;}if(el.matches('[data-chat-menu]')){openQuickMenu();return;}}
  function openQuickMenu(){if(!state.current)return;const modal=$('#buddyModal'),user=state.current;modal.innerHTML=`<section class="buddy-dialog buddy-actions-menu" role="dialog" aria-modal="true" aria-label="Ações de ${esc(profileName(user))}"><h2>${esc(profileName(user))}</h2><nav><a href="profile.html?user=${encodeURIComponent(user.id)}">${infoIcons.profile}<span>Ver perfil</span></a><button class="danger" data-block="${user.id}"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="m6 18 12-12"/></svg><span>Bloquear</span></button><button data-report="${user.id}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3 2.5 20h19Z"/><path d="M12 9v5M12 17h.01"/></svg><span>Denunciar</span></button><button data-dialog-close><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m5 5 14 14M19 5 5 19"/></svg><span>Fechar</span></button></nav><div class="buddy-settings"><label class="buddy-switch"><span>Som das mensagens</span><input id="buddySound" type="checkbox" ${state.sound?'checked':''}></label></div></section>`;modal.hidden=false;}
  ...
  $('#buddyModal')?.addEventListener('click',event=>{if(event.target===$('#buddyModal'))$('#buddyModal').hidden=true;});
  document.addEventListener('keydown',event=>{if(event.key==='Escape'){if(!$('#buddyModal').hidden)$('#buddyModal').hidden=true;else if($('#buddyInfo').classList.contains('open'))toggleInfoPanel(false);}});
  window.addEventListener('load',()=>{const target=new URLSearchParams(location.search).get('user');if(!target)return;const started=Date.now();const waitForTarget=()=>{if(state.current?.id===target)return;const profile=state.profiles.get(target);if(profile&&friends&&friendState(target)==='buddy'){openChat(target);return;}if(Date.now()-started<10000)setTimeout(waitForTarget,100);};waitForTarget();});
  ...
  root.addEventListener('click',event=>{const el=event.target.closest('button');if(!el)return;if(el.matches('[data-contact-filter]')){state.contactFilter=el.dataset.contactFilter;document.querySelectorAll('[data-contact-filter]').forEach(button=>button.classList.toggle('active',button===el));renderContacts();}else if(el.matches('[data-contact-view]')){state.contactView=el.dataset.contactView;document.querySelectorAll('[data-contact-view]').forEach(button=>button.classList.toggle('active',button===el));renderContacts();}else if(el.matches('#buddySearchClear')){$('#buddySearch').value='';state.searchResults=[];renderSidebar();$('#buddySearch').focus();}});
})();

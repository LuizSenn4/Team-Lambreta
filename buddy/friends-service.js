(() => {
  ...
  class FriendsService {
    ...
    async load() {
      ...
      const profiles = ids.size ? await this.client.from('profiles').select('id,game_nickname,full_name,avatar_url,custom_avatar_url,role,presence,last_seen,public_bio,main_game,profile_public').in('id', [...ids]) : { data: [], error: null };
    ...
    async search(query) {
      ...
      const result = await this.client.from('profiles')
        .select('id,game_nickname,full_name,avatar_url,custom_avatar_url,role,presence,last_seen,public_bio,main_game,profile_public')
    ...
    async getPublicProfile(id) {
      const result = await this.client.from('profiles').select('id,game_nickname,full_name,avatar_url,custom_avatar_url,role,presence,last_seen,public_bio,main_game,profile_public').eq('id', id).maybeSingle();
      if (result.error) throw result.error;

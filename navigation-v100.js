(() => {
  ...
  nav.querySelector('.tl-menu-toggle').onclick = event => { event.stopPropagation(); const group=event.currentTarget.closest('.tl-menu-group'); const open=!group.classList.contains('is-open'); closeFloating(); group.classList.toggle('is-open',open); event.currentTarget.setAttribute('aria-expanded',String(open)); };
  ...
  if(document.querySelector('.home-v22')&&!document.querySelector('.tl-home-portal')){const portal=document.createElement('section');portal.className='tl-home-portal';portal.innerHTML='<article class="tl-home-panel"><small>EM DESTAQUE</small><h3>Uma família dentro e fora do jogo</h3><p>Descobre as novidades, competições e histórias que movem a comunidade Team Lambreta.</p><a href="atualizacoes.html">Ver novidades →</a></article><article class="tl-home-panel"><small>COMUNIDADE</small><h3>Quem está online</h3><p>A presença real dos membros aparece no lobby e no Buddy.</p><a href="buddy.html">Abrir Buddy →</a></article><article class="tl-home-panel"><small>PARTICIPA</small><h3>Junta-te à Team</h3><p>Entra nas próximas formações e eventos competitivos.</p><a href="participe.html">Participar →</a></article>';document.querySelector('.home-v22').appendChild(portal)}
})();

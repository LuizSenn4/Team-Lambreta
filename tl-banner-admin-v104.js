(() => {
  ...
  const PRESETS = Object.freeze({
    desktop:{label:'Desktop', width:1920, height:600, minRatio:2.8, maxRatio:3.55},
    mobile:{label:'Mobile', width:1080, height:1350, minRatio:.68, maxRatio:.95}
  });
  ...
  let banners = committedBanners.map(item => ({...item, preview:'', mobilePreview:''}));
  ...
  const persist = async message => {
    ...
    try {
      ...
      const usedKeys = new Set(clean.flatMap(item => [item.imageKey, item.mobileImageKey]).filter(Boolean));
      const obsolete = new Set([
        ...committedBanners.flatMap(item => [item.imageKey, item.mobileImageKey]).filter(Boolean),
        ...pendingKeys
      ]);
  ...
  const processFile = (file, index, variant = 'desktop') => {
    const preset = PRESETS[variant] || PRESETS.desktop;
    ...
    if (!file || !typeOk) { report('TL-BANNER-003', 'Formato de banner inválido', {slot:index + 1, variant, type:file?.type, name:file?.name}); notify('Use PNG, JPG ou WebP.'); return; }
    if (file.size > MAX_INPUT_BYTES) { report('TL-BANNER-004', 'Ficheiro de banner excede o limite de processamento', {slot:index + 1, variant, bytes:file.size}); notify('A imagem excede 12 MB e não pode ser processada.'); return; }
    if (file.size > MAX_BYTES) notify(`Imagem acima de 2 MB (${(file.size / 1048576).toFixed(1)} MB); a comprimir antes do envio…`);

    const reader = new FileReader();
    reader.onerror = error => { report('TL-BANNER-005', 'Falha ao ler imagem do banner', {slot:index + 1, variant}, error); notify('Não foi possível ler a imagem.'); };
    reader.onload = () => {
      const image = new Image();
      image.onerror = error => { report('TL-BANNER-005', 'Falha ao preparar imagem do banner', {slot:index + 1, variant}, error); notify('Não foi possível preparar a imagem.'); };
      image.onload = async () => {
        const targetRatio = preset.width / preset.height;
        const ratio = image.width / image.height;
        if (ratio < preset.minRatio || ratio > preset.maxRatio) {
          report('TL-BANNER-006', 'Proporção de banner fora do recomendado', {slot:index + 1, variant, width:image.width, height:image.height, ratio, targetRatio});
          notify(variant === 'mobile' ? 'Use uma imagem vertical próxima de 1080×1350px (4:5).' : 'Use uma imagem horizontal próxima de 1920×600px.');
          return;
        }
        ...
        const key = `banner-${index + 1}/${variant}-${Date.now()}-${Math.random().toString(36).slice(2,8)}.webp`;
        notify(`A enviar Banner ${index + 1} ${preset.label} para a nuvem…`);
        try {
          ...
          const previousPending = banners[index][keyField];
          ...
          banners[index][keyField] = key;
          banners[index][urlField] = '';
          banners[index][previewField] = storage.getPublicUrl(key);
          banners[index].active = true;
          pendingKeys.add(key);
          render();
          setFeedback(`Banner ${index + 1} ${preset.label} enviado. Clique em “Salvar banners” para publicar.`);
        } catch (error) {
          report('TL-BANNER-023', 'Não foi possível enviar o banner', {slot:index + 1, variant, key}, error);
  ...
  function bind() {
    root.querySelectorAll('[data-select]').forEach(button => button.addEventListener('click', () => root.querySelector(`[data-file="${button.dataset.select}"][data-variant="${button.dataset.variant}"]`)?.click()));
    ...
    root.querySelectorAll('[data-dropzone]').forEach(zone => {
      ...
      const variant = zone.dataset.variant || 'desktop';
      zone.addEventListener('dragover', event => { event.preventDefault(); zone.classList.add('is-dragover'); });
      zone.addEventListener('dragleave', () => zone.classList.remove('is-dragover'));
      zone.addEventListener('drop', event => { event.preventDefault(); zone.classList.remove('is-dragover'); processFile(event.dataTransfer.files?.[0], index, variant); });
      zone.addEventListener('keydown', event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); root.querySelector(`[data-file="${index}"][data-variant="${variant}"]`)?.click(); } });
      zone.addEventListener('click', () => root.querySelector(`[data-file="${index}"][data-variant="${variant}"]`)?.click());
    });
  }
  ...
  const init = async () => {
    ...
    banners = committedBanners.map(item => ({...item, preview:storage.resolveUrl(item), mobilePreview:item.mobileImageKey || item.mobileUrl ? storage.resolveMobileUrl(item) : ''}));
    render();

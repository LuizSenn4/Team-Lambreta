(() => {
  ...
  function openModal(row){
    ...
    content.innerHTML=`<div class="team-profile-mobile-hero"><div class="team-esports-profile-photo">${avatar(row)}</div></div><div class="team-esports-profile-copy"><div class="team-profile-mobile-heading"><p class="team-profile-status">ONLINE</p><h2 id="teamProfileName">${esc(nickname)}</h2><div class="team-profile-heading-meta">${rolesMarkup(row.role||'MEMBRO')}<span class="team-profile-country"><span aria-hidden="true">${flag}</span> ${esc(row.country||countryCode)}</span></div></div><p class="tag">PERFIL OFICIAL</p><h2 class="team-profile-desktop-name">${esc(nickname)}</h2><div class="team-profile-rich-role team-profile-desktop-role">${rolesMarkup(row.role||'MEMBRO')}</div><div class="team-profile-metrics">${metrics.map(([label,value])=>`<div><small>${esc(label)}</small><strong>${esc(value)}</strong></div>`).join('')}</div>${factMarkup?`<div class="team-esports-facts">${factMarkup}</div>`:''}<section class="team-profile-section"><small>Sobre</small><div class="team-esports-bio team-profile-rich-bio">${richText(row.bio||'Perfil oficial do Team Lambreta.')}</div></section><section class="team-profile-section"><small>Cargos</small><div class="team-profile-role-list">${rolesMarkup(row.role||'MEMBRO')}</div></section>${specialties.length?`<section class="team-profile-section"><small>Especialidades</small><div class="team-profile-specialties">${specialties.map(item=>`<span>${esc(item)}</span>`).join('')}</div></section>`:''}${linkMarkup?`<section class="team-profile-section"><small>Redes sociais</small><div class="team-esports-links">${linkMarkup}</div></section>`:''}</div>`;

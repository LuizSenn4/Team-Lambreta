(() => {
  ...
  async function renderEditor() {
    ...
    root.innerHTML = `<div class="tl-page-heading"><small>MINHA CONTA</small><h1>Editar perfil</h1><p>Esta identidade será utilizada no header, Buddy, Fórum e restantes áreas da Team Lambreta.</p></div><div class="profile-editor-v102"><form id="profileForm" class="tl-v102-card profile-form-v102">
      ...
      <div class="profile-field-v102"><label for="profileCountrySearch">País</label><div class="profile-country-combobox"><input class="tl-v102-input" id="profileCountrySearch" type="search" role="combobox" aria-autocomplete="list" aria-controls="profileCountryList" aria-expanded="false" autocomplete="off" placeholder="Pesquisar por sigla ou país…" value="${esc(window.TeamCountryCatalog?.resolve(profile?.country)?.name || profile?.country || '')}"><input type="hidden" name="country" value="${esc(window.TeamCountryCatalog?.resolve(profile?.country)?.name || profile?.country || '')}"><div id="profileCountryList" class="profile-country-list" role="listbox" hidden></div></div><small>Pesquisa por sigla, nome em português, inglês ou nome comum.</small></div>
    ...
    const chooseCountry = country => { countryInput.value = country.name; form.elements.country.value = country.name; closeCountryList(); countryInput.focus(); };

(() => {
  ...
  function modesFact(profile, gameLabels) {
    const groups = new Map();
    (profile?.game_modes || []).forEach(value => {
      const [gameSlug, ...modeParts] = String(value || '').split('::');
      const mode = modeParts.join('::').trim();
      if (!gameSlug || !mode) return;
      if (!groups.has(gameSlug)) groups.set(gameSlug, []);
      groups.get(gameSlug).push(mode);
    });
    if (!groups.size) return fact('Modos', '—', 'modes');
    const content = [...groups].map(([gameSlug, modes]) => `<section class="profile-mode-group-v102"><h3>${esc(gameLabels.get(gameSlug) || readableSlug(gameSlug))}</h3><div>${modes.map(mode => `<span>${esc(mode)}</span>`).join('')}</div></section>`).join('');
    return `<div class="profile-fact-v102 is-modes"><span class="profile-fact-icon-v102">${profileIcons.modes}</span><dt>Modos</dt><dd class="profile-mode-groups-v102">${content}</dd></div>`;
  }
